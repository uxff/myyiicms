...
CREATE TABLE `#@__goods` (
  ...
  `catalog_id` int(10) unsigned NOT NULL DEFAULT '' COMMENT '栏目id',
...
CREATE TABLE `#@__page` (
  ...
  `update_time` int(10) unsigned NOT NULL DEFAULT '' COMMENT '更新时间',
  PRIMARY KEY (`id`)
...
CREATE TABLE `#@__question` (
  ...
  `client_ip` varchar(15) NOT NULL DEFAULT '' COMMENT 'ip',
...
CREATE TABLE `#@__video` (
  ...
  `down_count` smallint(8) unsigned DEFAULT '0' COMMENT '下载次数',
  `voted` varchar(100) NOT NULL DEFAULT '0' COMMENT '投票结果',
  `status` enum('Y','N') NOT NULL DEFAULT 'Y' COMMENT '是否显示',
  `seo_title` varchar(255) NOT NULL DEFAULT '' COMMENT 'SEO标题',
  `seo_description` text COMMENT 'SEO描述',
  `seo_keywords` varchar(255) NOT NULL DEFAULT '' COMMENT 'SEO关键字',
